...
class HandLandMarkViewModelTest {
    ...
    @Mock
    private lateinit var mockRepository: HandLandMarkRepository
    ...
    @Mock
    private lateinit var mockHandLandmarkerResult: HandLandmarkerResult
    ...
    fun `processImageProxyThrottled updates landmarks when successful`() = runTest {
        ...
        doAnswer { invocation ->
            ...
            null
        }.`when`(mockRepository).processImageProxyThrottled(
            anyOrNull(),
            any(),
            any()
        )
