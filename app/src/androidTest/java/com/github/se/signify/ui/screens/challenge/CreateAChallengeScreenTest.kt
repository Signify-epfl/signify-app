...
class CreateAChallengeScreenTest {
  ...
  private val testUser = User(uid = "testUserId", name = "Test User")
  ...
  fun setUp() {
    ...
    // Mock getFriendsList method to return friends list
    doAnswer { invocation ->
          val onSuccess = invocation.arguments[1] as (List<String>) -> Unit
          onSuccess(friends)
        }
        .`when`(userRepository)
        .getFriendsList(eq(testUser.uid), any(), any())
    ...
    // Simulate fetching friends list from ViewModel
    userViewModel.getFriendsList(testUser.uid)
  }
