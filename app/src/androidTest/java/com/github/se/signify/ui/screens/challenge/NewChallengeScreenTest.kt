...
class NewChallengeScreenTest {
  ...
  private val testUser =
      User(
          uid = "testUserId",
          name = "Test User",
          ongoingChallenges = listOf("challenge1", "challenge2"))
  ...
  fun setUp() {
    ...
    // Mock getOngoingChallenges to update the ongoingChallenges state in the ViewModel
    doAnswer { invocation ->
          val onSuccess = invocation.arguments[1] as (List<Challenge>) -> Unit
          onSuccess(ongoingChallenges) // This triggers ViewModel's _ongoingChallenges update
        }
        .`when`(userRepository)
        .getOngoingChallenges(eq(testUser.uid), any(), any())

    // Mock removeOngoingChallenge
    doAnswer { invocation ->
    ...
    // Simulate calling getOngoingChallenges from within LaunchedEffect in the UI
    userViewModel.getOngoingChallenges(testUser.uid)
  }
